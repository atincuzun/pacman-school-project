...
public class JavaUI extends Application{
    ...
    public void start(Stage stage) throws Exception {
        ...
        ghostIMG = new Image(new FileInputStream("D:\\ghost.png"));
...
!cannot use tilepane due to automatic sortition of images -> fill
    	...
    	gamePane.setMaxWidth(screenWidth/3);
    	gamePane.setMaxHeight(screenHeight/3);
    	gamePane.setStyle("-fx-background-color: #000000");
    	
    	Pane gamePanel = new Pane();
    	for(int i = 0; i < 3; i++) {
    		ImageView aa = new ImageView(ghostIMG);
    		aa.setFitHeight(50);
    		aa.setFitWidth(50);
    	    aa.setLayoutX((i*gamePane.getWidth())/mapSizeX);

			gamePane.getChildren().add(aa);
    		for(int l = 0; l < 2; l++) {
        		ImageView sasa = new ImageView(ghostIMG);
        		sasa.setFitHeight(50);
        		sasa.setFitWidth(50);
        	    sasa.setLayoutX(l*10);
        		gamePane.getChildren().add(sasa);
    	...
    	//gamePane.setPadding(new javafx.geometry.Insets(10, 10, 10, 10)); 
    	//gamePane.add(pacmanIV, 0, 1);

    	//gamePane.add(ghostIV, 1, 111);
		...
		stage.widthProperty().addListener((obs, oldVal, newVal) -> {
			...
			for(int i = 0; i < gamePane.getChildren().size(); i++) {
				if(gamePane.getChildren().get(i) instanceof ImageView) {
					...
					((ImageView) gamePane.getChildren().get(i)).setFitWidth(gamePane.getWidth()/16);
					((ImageView) gamePane.getChildren().get(i)).setLayoutX((i*gamePane.getWidth())/mapSizeX);
					//gamePane.getWidth()/mapSizeX + equalizer
					equalizer += (gamePane.getWidth()/mapSizeX);
				}
				
			}
			
			gamePane.setMaxWidth(stage.getWidth()/1.2);
		...
		stage.heightProperty().addListener((obs, oldVal, newVal) -> {
			...
			for(int i = 0; i < gamePane.getChildren().size(); i++) {
				if(gamePane.getChildren().get(i) instanceof ImageView) {
					//((ImageView) gamePane.getChildren().get(i)).setFitWidth(stage.getWidth()/20);
					//((ImageView) gamePane.getChildren().get(i)).setLayoutY(i*10);

				}
				
			}
			gamePane.setMaxHeight(stage.getHeight()/1.2);
    ...
    public int i = 0;
    public void MoveTheGuy(ImageView imgView) {
    	i+=5;
    	imgView.relocate(2+i, 2+i);
    	imgView.setX(i);
