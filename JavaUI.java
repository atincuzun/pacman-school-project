...
public class JavaUI extends Application{
	...
	private double excessStageY = 0;
	private double excessStageX = 0;
	...
	private double gameWideBlockHeight = 0;
	private double gameWideBlockWidth = 0;
	
    public double getExcessStageY() {
		return excessStageY;
	}

	public void setExcessStageY(double excessStageY) {
		this.excessStageY = excessStageY;
	}
    public double getExcessStageX() {
		return excessStageX;
	}

	public void setExcessStageX(double excessStageX) {
		this.excessStageX = excessStageX;
	}
	public void start(Stage stage) throws Exception {
        ...
        pacmanIV.setPreserveRatio(false);
...
!cannot use tilepane due to automatic sortition of images -> fill
        ...
        map.GenerateMap();
        excessStageY = map.getExcessStageY();
        excessStageX = map.getExcessStageX();

		stage.widthProperty().addListener(e -> {
			double newSizeWidth = stage.getWidth() - excessStageX;
			for(int i = 0; i < mapSizeX; i++) {
				for(int a = 0; a < mapSizeY; a++) {
					if(gamePane.getChildren().get(i) instanceof ImageView) {
						...
						map.getMap()[i][a].setX((i*newSizeWidth)/mapSizeX);
						map.getMap()[i][a].setFitWidth(newSizeWidth/mapSizeX);
						//gamePane.getWidth()/mapSizeX + equalizer
						gameWideBlockWidth = newSizeWidth/mapSizeX;

					}	
				}
			}
			
			//Set position of pacman relative to the map blocks so when resized
			//it wont overflow the cells
	        //pacmanIV.setX(map.getData().length );
	        pacmanIV.setFitWidth(gameWideBlockWidth);
			
		});

		stage.heightProperty().addListener(e -> {
			double newSizeHeight = stage.getHeight() - excessStageY;
			for(int i = 0; i < mapSizeX; i++) {
				for(int a = 0; a < mapSizeY; a++) {
					if(gamePane.getChildren().get(i) instanceof ImageView) {
						//System.out.println(ratio);
						//! really important -> set fitwidth as the total amount of object on one row
						map.getMap()[i][a].setY((a*newSizeHeight)/mapSizeY);
						map.getMap()[i][a].setFitHeight(newSizeHeight/mapSizeY);
						//gamePane.getWidth()/mapSizeX + equalizer
						gameWideBlockHeight = newSizeHeight/mapSizeY;
					}	
				}
			}
			pacmanIV.setFitHeight(gameWideBlockHeight);
		});
		gameWideBlockHeight = (stage.getHeight() - excessStageY)/mapSizeY;
		gameWideBlockWidth = (stage.getWidth() - excessStageX)/mapSizeX;
		pacmanIV.setFitHeight(gameWideBlockHeight);
        pacmanIV.setFitWidth(gameWideBlockWidth);
        System.out.println(gameWideBlockHeight + ", " + gameWideBlockWidth);
        pacmanIV.toFront();
...
class Map extends JavaUI {
	...
	public void PopulateMap() {
		...
		for(int i = 0; i < mapSizeY; i++) {
			data[mapSizeX][i] = wallNumber;
		}
		for(int i = 0; i < mapSizeX; i++) {
			data[i][0] = wallNumber;
		}
		for(int i = 0; i < mapSizeX; i++) {
			data[i][mapSizeY] = wallNumber;
		}
	}
	...
	public Pane GenerateMap() {
		//System.out.println(gamePane.getHeight() + " " + stage.getHeight());
		super.setExcessStageY(stage.getHeight() - gamePane.getHeight());
		super.setExcessStageX(stage.getWidth() - gamePane.getWidth());
		//System.out.println(super.getRatio());
    	for(int i = 0; i < mapSizeX; i++) {
    		for(int a = 0; a < mapSizeY; a++) {
